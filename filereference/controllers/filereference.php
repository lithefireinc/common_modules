...
class Filereference extends MY_Controller{
	...
	function getSubjectCombo(){
       ...
       // die($this->db->last_query());
        ...
        if($records){
        foreach($records as $row):
			$row['name'] = $row['id']." (".$row['SUBJCODE'].") ".$row['COURSEDESC'];
            $temp[] = $row;
            $total++;
	...
	function getFacilityItems(){
        ...
        $records = array();
        $table = "FACIITEM";
        $fields = array("ITEMCODE", "ITEMIDNO", "ITEM", "DESCRIPTION");
	...
	{
			$data['title'] = "Residency | HR";
			$data['userId'] = $this->session->userData('userId');
			$data['userName'] = $this->session->userData('userName');
			$this->layout->view('filereference/FILERESI_view', $data);
	}

		function getFILERESI(){
	        ...
	        $records = array();
	        $table = "FILERESI";
	        $fields = array("RESICODE","RESIIDNO","RESIDENCY","DCREATED","TCREATED","DMODIFIED","TMODIFIED",);
	        $db = 'fr';
	        $filter = "";
	        $group = "";
			if(empty($sort) && empty($dir)){
			...
			if(!empty($query)){
 				"(RESICODE LIKE '%$query%' OR RESIIDNO LIKE '%$query%' OR RESIDENCY LIKE '%$query%' OR DCREATED LIKE '%$query%' OR TCREATED LIKE '%$query%' OR DMODIFIED LIKE '%$query%' OR TMODIFIED LIKE '%$query%')";
	    	}
		...
		function addFILERESI(){
	        ...
	        //uncomment for FRs
			$input['RESIIDNO'] = $this->lithefire->getNextCharId($db, $table, 'RESIIDNO', 5);
		...
		function updateFILERESI(){
			...
			$fieldname = 'RESIDENCY';
	        if($this->lithefire->countFilteredRows($db, $table, "$fieldname = '".$this->input->post("$fieldname")."' AND RESICODE != '$id'", "")){
	            $data['success'] = false;
	            $data['data'] = "Record already exists";
	            die(json_encode($data));
	        }
