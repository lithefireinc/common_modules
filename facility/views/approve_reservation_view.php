 ...
 reservation.app = function()
 {
 	return{
 		...
 		getGrid: function()
 		{
 			...
 			var Objstore = new Ext.data.Store({
 						proxy: new Ext.data.HttpProxy({
 							url: "<?=site_url("facility/getAllReservations")?>",
 							method: "POST"
 							}),
 			...
 			var grid = new Ext.grid.GridPanel({
 				...
 				cm:  new Ext.grid.ColumnModel(
                         ...
                         { header: "Confirmation No.", width: 120, sortable: true, dataIndex: "CONFIRMNO" },
 						 ...
 						 { header: "Requested By", width: 100, sortable: true, dataIndex: "REQUESTED_BY" },
 		...
 		Edit: function(){
 			...
 			var dateto = sm.getSelected().data.DATETO;
 		    ...
 		    _window = new Ext.Window({
 		        ...
 		        buttons: [{
 		            text: 'Approve',
                            icon: '/images/icons/accept.png', cls:'x-btn-text-icon',
					disabled: true,
					id: 'approve_button',
 		            handler: function(){
 			            if(ExtCommon.util.validateFormFields(reservation.app.Form)){//check if all forms are filled up
 		                reservation.app.Form.getForm().submit({
 			                url: "<?=site_url("facility/approve")?>",
 			                params: {CONFIRMNO: id},
 			                method: 'POST',
 			                success: function(f,action){
                 		    	Ext.Msg.show({
 		        ...
 		        {
 		            text: 'Deny',
                            icon: '/images/icons/cross.png', cls:'x-btn-text-icon',
					disabled: true,
					id: 'deny_button',
 		            handler: function(){
 			            if(ExtCommon.util.validateFormFields(reservation.app.Form)){//check if all forms are filled up
 		                reservation.app.Form.getForm().submit({
 			                url: "<?=site_url("facility/deny")?>",
 			                params: {CONFIRMNO: id},
 			                method: 'POST',
 			                success: function(f,action){
                 		    	Ext.Msg.show({
 		    ...
 		    //alert(status);
 		    if(status == 'Pending'){
 				Ext.getCmp("approve_button").enable();
 				Ext.getCmp("deny_button").enable();
 			}
 			//var now = new Date();
			var now = new Date();
			//alert(iso_date.format("Y-m-d")); // alerts 01.11.2008 22:00:00
			//alert(now+" "+dateto);
 			if(status == 'Approved' && now.format("Y-m-d") <= dateto){
 				//Ext.getCmp("approve_button").enable();
 				Ext.getCmp("deny_button").enable();
 			}
